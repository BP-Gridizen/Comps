# -*- coding: utf-8 -*-
"""
This will hold all of the useful functionality for the automated valuation model. DO NOT EDIT.
Created on Thu Aug  9 10:26:27 2018
...
def data_load(website, parameters, min_beds, max_beds, search_type):
    """ this uses the parameters dictionary defined in parameter_Def to make an API call to Zoopla. This then parses the json into a dataframe file """
    frames = []
    ...
    for dic in parameters:
        ...
        if status_code != 200:
            ...
            try:
                data_frame = data_frame.ix[~(data_frame['price']==0)]
...
def automatic_filter(dataframe, bounds, filter_flag,min_beds, max_beds):
    "This uses the earlier defined dataframe and bounds to automatically filter the data. Filter_flag is used to determine whether the user determines if a property is dropped or not"""
    ...
    if filter_flag == "N":
        for index, row in dataframe.iterrows():
            ...
            if row['price'] > upper_bounds[bedroom_index]:
                ...
                print('Address - {address}, Price £{price} ,{beds} beds, Property Type: {ptype}, Described as: {description}'.format(address = row['displayable_address'], price = row['price'], beds = row['num_bedrooms'], ptype = row['property_type'], description = row['description'])) 
...
def plot_title(post_code, radius, min_beds, max_beds, search_type):
    ...
    title = "{} Comps Analysis for {}-{} bedroom properties within a {} mile radius of {} {}".format(comps_flag, min_beds, max_beds, radius, post_code[:-3].upper(), post_code[-3:].upper()) 
    return title

def plot_boxwhisker(dataframe, title_string, search_type, post_code, mkdir_output, save_flag):
...
#    ax.get_xaxis().set_major_formatter(matplotlib.ticker.FuncFormatter(lambda x, p: format(int(x), ',')))
    ...
    if save_flag == "Y":
        save_plots(post_code, plot_type,mkdir_output)    
    return

def plot_seaborn_dist(dataframe,minimum_beds, maximum_beds, title_string, search_type, post_code, mkdir_output, save_flag):
    values = dataframe.loc[:,['num_bedrooms', 'price']]
...
def save_plots(post_code, plot_type, mkdir_outputs):
    os.chdir("../Comps/"+mkdir_outputs[0])
...
def full_comps_calc(website, post_code, radius, minimum_beds, maximum_beds, comps_type, percentile, filter_flag, save_flag):
    ...
    plot_boxwhisker(filtered_data_frame, title, comps_type, post_code, mkdir_outs, save_flag)
    plot_seaborn_dist(filtered_data_frame, minimum_beds, maximum_beds, title, comps_type, post_code, mkdir_outs, save_flag)
    if save_flag == "Y":
        save_to_csv(filtered_data_frame, comps_type, post_code, mkdir_outs)
    return comps

def save_to_csv(dataframe, search_type, post_code, mkdir_output):
    path_name = mkdir_output[0]
    date_string = mkdir_output[1]
    os.chdir("../Comps/"+path_name)
    os.chdir(date_string)
